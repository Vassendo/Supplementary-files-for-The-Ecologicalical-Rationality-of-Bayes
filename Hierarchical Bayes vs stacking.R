#These are the second two simulations performed in Section 3 of the paper
...
run <- foreach(trials = 1:n_trials) %dorng% {
  ...
  for(j in 1:7){
    ...
    bayes_model <- sampling(stan_object,
                            data = list(likelihood = likelihood, N = num_outcomes, K = K, alpha = c(1, 1, 1, 1)),
                            iter = 2000,
                            warmup = 1000,
                            thin = 1,
                            chains = 4) 
    ...
    bayes_weights <- colMeans(rstan::extract(bayes_model)[['beta']])
    stack_weights <- divergence_weights(likelihood, prior = prior)
    
    
    bayes_prediction <- as.numeric(hyp%*%bayes_weights)
...
misspecified_hierarchical_plot <- ggplot(results.tidy, aes(x = data, y = Log_score, col = Updating_rule)) + 
...
wellspecified_hierarchical_plot <- ggplot(results.tidy, aes(x = data, y = Log_score, col = Updating_rule)) + 
  geom_point(shape = 1, alpha = 0.5) + 
  coord_fixed(
    ratio = 1000,
    ylim = c(0.42, 0.62),
